...
import org.jmin.beecp.pool.exception.PoolClosedException;
...
public final class FastConnectionPool extends Thread implements ConnectionPool, ConnectionPoolJmxBean, PooledConnectionValidTest, PooledConnectionTransferPolicy {
    ...
    private void registerJmx() {
        if (this.poolConfig.isEnableJmx()) {
            ...
            this.registerJmxBean(mBeanServer, String.format("org.jmin.beecp.pool.FastConnectionPool:type=BeeCP(%s)", this.poolName), this);
            this.registerJmxBean(mBeanServer, String.format("org.jmin.beecp.BeeDataSourceConfig:type=BeeCP(%s)-config", this.poolName), this.poolConfig);
        }
    }
    ...
    private void unregisterJmx() {
        if (this.poolConfig.isEnableJmx()) {
            MBeanServer mBeanServer = ManagementFactory.getPlatformMBeanServer();
            this.unregisterJmxBean(mBeanServer, String.format("org.jmin.beecp.pool.FastConnectionPool:type=BeeCP(%s)", this.poolName));
            this.unregisterJmxBean(mBeanServer, String.format("org.jmin.beecp.BeeDataSourceConfig:type=BeeCP(%s)-config", this.poolName));
        }
    }
