...
import org.jmin.concurrent.synchronizer.PermitWaitNode;
import org.jmin.concurrent.synchronizer.PermitWaitNodeChain;
import org.jmin.concurrent.synchronizer.PermitWaitNodeState;
 ...
 * @version 1.0
 */
public class SynchronizeWaitChain implements PermitWaitNodeChain {
    //***************************************************************************************************************//
    //                                           1: CAS Chain info                                                   //
    //***************************************************************************************************************//
    ...
    protected transient volatile PermitWaitNode head = new PermitWaitNode(PermitWaitNodeState.EMPTY);
    protected transient volatile PermitWaitNode tail = new PermitWaitNode(PermitWaitNodeState.EMPTY);
    ...
    public int getLength() {
        ...
        for (PermitWaitNode node = head.getNext(); node != null; node = node.getNext()) {
            ...
            if (state == PermitWaitNodeState.WAIT_FOR_EXCLUSIVE || state == PermitWaitNodeState.WAIT_FOR_SHARE || state == PermitWaitNodeState.RETRY_ACQUIRE)
                size++;
        }
        return size;
    }
    ...
    public boolean removeNode(PermitWaitNode node) {
        ...
        for (PermitWaitNode curNode = firstNode; curNode != null; prevNode = curNode, curNode = curNode.getNext()) {
            ...
            if (state == PermitWaitNodeState.EMPTY) {
                ...
                if (curNode == node) {
                    ...
                    removed = casNodeState(curNode, state, PermitWaitNodeState.EMPTY);//logic remove
                }
