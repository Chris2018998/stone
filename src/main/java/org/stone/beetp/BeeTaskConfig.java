...
public final class BeeTaskConfig {
    ...
    private long periodDelayTime;
    private boolean fixedRateDelay;
    ...
    public long getPeriodDelayTime() {
        return periodDelayTime;
    }

    public void setPeriodDelayTime(long periodDelayTime) {
        this.periodDelayTime = periodDelayTime;
    }

    public boolean getFixedRateDelay() {
        return fixedRateDelay;
    }

    public void setFixedRateDelay(boolean fixedRateDelay) {
    ...
    public void check() throws TaskConfigException {
        ...
        if (initDelayTime < 0)
            throw new TaskConfigException("Initialization delay time can't be less than zero");
        if (periodDelayTime < 0)
            throw new TaskConfigException("Period time must be greater than zero");
        if ((initDelayTime > 0 || periodDelayTime > 0) && timeUnit == null)
