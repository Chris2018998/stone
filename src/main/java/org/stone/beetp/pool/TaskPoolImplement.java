...
public final class TaskPoolImplement implements BeeTaskPool {
    ...
    private OnceFactory onceExecFactory;
    private JoinFactory joinExecFactory;
    private ScheduledFactory scheduledExecFactory;
    ...
    private void startup(BeeTaskServiceConfig config) {
        ...
        if (joinExecFactory == null) {
            this.joinExecFactory = new JoinFactory(this);
            this.onceExecFactory = new OnceFactory(this);
            this.scheduledExecFactory = new ScheduledFactory(this);
        }
    }
                ...
                if (handle != null) {
                    if (handle.setAsRunning()) {
                        if (handle instanceof OnceTaskHandle) {
                            taskHoldingCount.decrementAndGet();
                            onceExecFactory.execute((OnceTaskHandle) handle);
