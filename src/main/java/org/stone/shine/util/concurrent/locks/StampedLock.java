 ...
 *
 * 阴阳锁特点
 * 1：锁值是一个64位数值，并分为高32位和低32位，高32为记录锁的类型和重入次数（针对读），低32位为阴阳核心转换区
 * 2：32低位值是偶数为阴锁（写锁，阴意味：阴冷的，下降的）；为奇数时则为阳锁（读锁，阳意味：活波的，热气的，上升的）
 * 3：锁转换，低位数值加1，锁发生转换（阳转阴，阴转阳）；加2则获得下一个同类锁（1+2=3,2+2=4），阴阳（奇偶）可以一直转换下去。
 *
 * @author Chris Liao
 * @version 1.0
 */
