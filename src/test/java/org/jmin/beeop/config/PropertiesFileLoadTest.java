...
import org.jmin.beeop.pool.ObjectPoolStatics;
...
public class PropertiesFileLoadTest extends TestCase {
    ...
    private static String checkInterface(BeeObjectSourceConfig config) {
        if (!"org.jmin.beeop.object.JavaBook".equals(config.getObjectClassName())) return "objectClassName error";
        //if(!"JavaBook".equalsString(config.getObjectClass().getClass().getName()))return"objectClass error";
        if (!"org.jmin.beeop.object.JavaBookFactory".equals(config.getObjectFactoryClassName()))
            return "objectFactoryClassName error";
        if (!"org.jmin.beeop.pool.FastObjectPool".equals(config.getPoolImplementClassName()))
            return "objectFactoryClassName error";
