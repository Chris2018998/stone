 ...
 */
package org.stone.study.shine.util.concurrent.locks;

import org.stone.study.shine.util.concurrent.synchronizer.*;
import org.stone.study.shine.util.concurrent.synchronizer.chain.SyncNodeStates;
...
import static org.stone.study.shine.util.concurrent.synchronizer.SyncConstants.*;
import static org.stone.study.shine.util.concurrent.synchronizer.validator.ResultEqualsValidator.BOOL_EQU_VALIDATOR;
...
class BaseLock implements Lock {
    ...
    BaseLock(boolean fair, LockAction lockAction) {
        this(new ResultWaitPool(fair), lockAction, TYPE_EXCLUSIVE);
    }

    //constructor2(extend by WriteLockImpl,ReadLockImpl)
    BaseLock(ResultWaitPool waitPool, LockAction lockAction, Object acquireType) {
    ...
    public boolean tryLock() {
        return lockAction.tryAcquire(1);
    }
     ...
     */
    public void unlock() {
        if (lockAction.tryRelease(1)) waitPool.wakeupFirst();
    }
    ...
    static class LockConditionImpl implements Condition {
        ...
        private void doAwait(SyncVisitConfig config) throws InterruptedException {
            ...
            if (lockAction.tryRelease(holdCount)) lock.waitPool.wakeupFirst();
