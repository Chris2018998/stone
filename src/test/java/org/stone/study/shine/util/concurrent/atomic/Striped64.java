 ...
 */
package org.stone.study.shine.util.concurrent.atomic;
...
abstract class Striped64 extends Number {
    ...
    static boolean casCell(long x, Cell c, LongBinaryOperator fn) {
        ...
        return UNSAFE.compareAndSetLong(c, cellValueOffset, v, fn.applyAsLong(v, x));
    }

    static boolean casCell(double x, Cell c, DoubleBinaryOperator fn) {
        final long v = c.value;
        final double v2 = Double.longBitsToDouble(v);
        return UNSAFE.compareAndSetLong(c, cellValueOffset, v, Double.doubleToRawLongBits(fn.applyAsDouble(v2, x)));
    }

    private boolean casCellsBusy() {
        return UNSAFE.compareAndSetInt(this, cellsBusyOffset, 0, 1);
    }
