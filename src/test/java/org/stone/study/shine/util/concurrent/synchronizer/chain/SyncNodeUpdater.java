 ...
 */
package org.stone.study.shine.util.concurrent.synchronizer.chain;
...
public final class SyncNodeUpdater {
    ...
    static boolean casTail(SyncNodeChain chain, SyncNode expect, SyncNode update) {
        return chain.tail == expect && U.compareAndSetReference(chain, tailOffSet, expect, update);
    }

    public static boolean casPrev(SyncNode n, SyncNode curPrev, SyncNode newPrev) {
        return n.prev == curPrev && U.compareAndSetReference(n, prevOffSet, curPrev, newPrev);
    }

    public static boolean casNext(SyncNode n, SyncNode curNext, SyncNode newNext) {
        return n.next == curNext && U.compareAndSetReference(n, nextOffSet, curNext, newNext);
    }

    public static boolean casState(SyncNode n, Object expect, Object update) {
        return n.state == expect && U.compareAndSetReference(n, stateOffSet, expect, update);
    }

    public static boolean putState(SyncNode n, Object expect, Object update) {
        if (n.state == expect) {
            U.putReferenceRelease(n, stateOffSet, update);
            return true;
        } else {
            return false;
        }
