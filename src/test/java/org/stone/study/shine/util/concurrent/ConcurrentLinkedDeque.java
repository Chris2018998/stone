...
public class ConcurrentLinkedDeque<E> extends AbstractCollection<E> implements Deque<E>, java.io.Serializable {
    ...
    private static boolean casItemToNull(Node node, Object cmp) {
        return U.compareAndSetReference(node, itemOffSet, cmp, null);
    }

    private static boolean casTailNext(Node t, Node newNext) {
        return U.compareAndSetReference(t, nextOffSet, null, newNext);
    }

    private static boolean casHeadPrev(Node h, Node newPrev) {
        return U.compareAndSetReference(h, prevOffSet, null, newPrev);
    }
    ...
    private static void linkNextTo(Node startNode, Node endNode) {
        ...
        if (curNext != endNode) U.compareAndSetReference(startNode, nextOffSet, curNext, endNode);

        //endNode.prev ------> startNode
        Node curPrev = endNode.prev;
        if (curPrev != startNode) U.compareAndSetReference(endNode, prevOffSet, curPrev, startNode);
    }
    ...
    private static void linkPrevTo(Node startNode, Node endNode) {
        ...
        if (curPrev != endNode) U.compareAndSetReference(startNode, prevOffSet, curPrev, endNode);

        //endNode.next ------> startNode
        Node curNext = endNode.next;
        if (curNext != startNode) U.compareAndSetReference(endNode, nextOffSet, curNext, startNode);
    }
