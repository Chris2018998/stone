 ...
 */
package org.stone.test.study.queue;
 ...
 * <p>
 * The logic of queue is from BeeCP(https://github.com/Chris2018998/stone)
 *
 * @author Chris Liao
 */
public class MySemaphore {
    ...
    //park min nanoSecond,spin min parkTime value
    ...
    /**
     * Acquires a synchronizer from this semaphore, if one becomes available
     * within the given waiting parkTime and the current thread has not
     * been {@linkplain Thread#interrupt interrupted}.
     *
     * @param timeout the maximum parkTime to wait for a synchronizer
     * @param unit    the parkTime unit of the {@code timeout} argument
     * @return {@code true} if a synchronizer was acquired and {@code false}
     * if the waiting parkTime elapsed before a synchronizer was acquired
    ...
    /**
     * Releases a synchronizer, returning it to the semaphore.
     */
    public void release() {
        sync.release();
    }
    ...
    private static abstract class Sync {
        ...
        protected static boolean transferToWaiter(int newCode, Borrower borrower) {
            int state;
            do {
    ...
    private static final class FairSync extends Sync {
        ...
        public void release() { //transfer synchronizer
            ...
            usingSize.decrementAndGet();//release synchronizer
        }
    }
