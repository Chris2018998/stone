...
import org.stone.test.study.queue.MyTransferQueue;
...
//                consumerSize));
    ...
    private static long testQueue(String queueName, Queue<TransferPacket> queue, int producerSize, int consumerSize) throws Exception {
        ...
        MyTransferQueueConsumer[] fstConsumers = null;
        ...
        if (queue instanceof BlockingQueue<TransferPacket> blockingQueue) {//Blocking Queue Consumers
            ...
            MyTransferQueue<TransferPacket> MyTransferQueue = (MyTransferQueue<TransferPacket>) queue;
            fstConsumers = new MyTransferQueueConsumer[consumerSize];
            for (int i = 0; i < consumerSize; i++) {
                fstConsumers[i] = new MyTransferQueueConsumer(MyTransferQueue, pollStartCountLatch, pollEndCountLatch);
        ...
        if (queue instanceof BlockingQueue<TransferPacket> blockingQueue) {//Blocking Queue Consumers
            ...
            MyTransferQueue<TransferPacket> MyTransferQueue = (MyTransferQueue<TransferPacket>) queue;
            for (int i = 0; i < producerSize; i++) {
                new MyTransferQueueOfferProducer(MyTransferQueue, existConsumerInd, producersDownLatch).start();
            }
        }
