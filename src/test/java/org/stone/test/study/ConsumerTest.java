...
//        timeList.add(testQueue("MyTransferQueue", new MyTransferQueue<Object>(), producerSize, consumerSize, operateSize));
    ...
    private static long testQueue(String queueName, Queue<Object> queue, int producerSize, int consumerSize, int operateSize) throws Exception {
        ...
        if (queue instanceof BlockingQueue<Object> blockingQueue) {//Blocking Queue Consumers
            ...
            MyTransferQueue<Object> myTransferQueue = (MyTransferQueue<Object>) queue;
            fstConsumers = new FstQConsumer[consumerSize];
            for (int i = 0; i < consumerSize; i++) {
                fstConsumers[i] = new FstQConsumer(startTime, myTransferQueue, consumersDownLatch, operateSize);
