...
public class ReadWriteLockReleaseTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        writeThread.start();
        if (!TestUtil.waitUtilWaiting(writeThread)) throw new IllegalStateException();
        ...
        readThead.start();
        if (!TestUtil.waitUtilWaiting(readThead)) throw new IllegalStateException();
        ...
        if (readThead.time < writeThread.time)
            throw new IllegalStateException("Read thread has got read-lock before Write thread");
    }
