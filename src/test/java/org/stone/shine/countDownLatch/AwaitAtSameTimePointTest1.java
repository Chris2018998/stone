...
import org.stone.shine.countDownLatch.threads.SameTimePointToAwaitThread;
...
public class AwaitAtSameTimePointTest1 extends TestCase {
    ...
    public void test() throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            sameTimePointWaitThreads[i] = new SameTimePointToAwaitThread(latch, timePointToAWait);
            sameTimePointWaitThreads[i].start();
        }

        //4: crate count down threads
        LockSupport.parkNanos(timePointToAWait - System.nanoTime());///wait for the same time(delay 5 seconds)
