...
public class TryAcquireTimeOutTest extends TestCase {
    ...
    public void test() throws Exception {
        ...
        AcquireMockThread mockThread = new AcquireMockThread(semaphore, "tryAcquire", 2, TimeUnit.SECONDS);
        mockThread.start();

        //3:park main thread 2 seconds and check mock thread state
        LockSupport.parkNanos(TimeUnit.SECONDS.toNanos(3));
        TestUtil.assertError("TryAcquire test expect value:%s,actual value:%s", false, mockThread.isAcquireSuccess());
